...
export async function createUser(data) {
  try {
    const parsedUser = userSchema.createUserSchema.parse(data);
  } catch (error) {
    if (error instanceof userSchema.z.ZodError) {
      throw new ConflictError("Dados inválidos");
    }
    throw error;
  }
...
export async function updateUser(userId, data) {
  try {
    const format = { id: userId, ...data };
    const parsedUser = userSchema.updateUserSchema.parse(format);
  } catch (error) {
    if (error instanceof userSchema.z.ZodError) {
      throw new ConflictError("Dados inválidos");
    }
